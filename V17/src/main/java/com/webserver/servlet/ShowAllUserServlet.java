...
public class ShowAllUserServlet extends HttpServlet{
 ...
 //System.out.println("ShowAllUserServlet:开始生成动态页面");
        logger.info("开始生成动态页面");//记录普通信息的
        //1.从user。dat文件中将所有用户信息读取出来
        List<User> list = new ArrayList<>();
        //将user。dat文件用户 读取后以User对象形式存入list集合
        try {
            ...
            for (int i = 0; i < raf.length()/100; i++) {
                ...
                e.printStackTrace();
                logger.error(e.getMessage(),e);//记录所有错误信息
            }
                for(User user:list){
        ...
        //2.1所有页面上显示的动态数据都要存入一个名为Context的类中
        Context context = new Context();
        /*
           向Context中添加要在页面中显示的数据，使用类似与Map的put方法。
           第一个参数为名字，第二个参数为存放的值，将来在页面上某个位置要显示这些数据时
           会在哪里标注这个名字，thymeleaf就会根据名字获取对应的值来进行显示了
       */
        ...
        /*
            ...
            该方法就是thymeleaf将模板页面与context中的数据进行结合来生成一个动态页面，返回值就是生成后
            的动态页面的源代码(html代码)
         */
